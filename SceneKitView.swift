...
struct SceneKitView: UIViewRepresentable {
    ...
    @Binding var angleMultiplyer: AbsoluteDirec
    @Binding var earthRotating : AbsoluteDirec
    @Binding var isMoonStopped : IsMoonStopped
    let baseNode: SCNNode
    let sceneView : SCNView
    
    init(angleMultiplyer: Binding<AbsoluteDirec>,earthRotating: Binding<AbsoluteDirec>, isMoonStopped : Binding<IsMoonStopped>) {
        print("초기화")
    ...
    func baseSetting(angleMultiplyer : AbsoluteDirec, earthRotating : AbsoluteDirec){
        ...
        moon.name = "moon"
        moon.position = SCNVector3(x:angleMultiplyer.rawValue % 2 == 0 ? 0.2 : -0.2 , y: 0, z: 0)
        
        moonRing.position = SCNVector3(x:0.0 , y: 0.02, z: 0)
        
        if isMoonStopped == .MOVE {
            rotateMoon(rotation: .pi*0.5,planet: moonRing, duration: 1)}
        moonRing.rotation = SCNVector4(0, 1, 0, .pi*(-0.5)*Float(earthRotating.rawValue-1) + .pi*(-0.5)*Float(angleMultiplyer.rawValue))
        ...
        earth.rotation = SCNVector4(0, 1, 0, .pi*(0.5)*Float(earthRotating.rawValue-1))
        // person initial
        ...
        if isMoonStopped == .STOP{
            ...
            moonRing.rotation = SCNVector4(0, 1, 0, .pi*(-0.5)*Float(earthRotating.rawValue-1) + .pi*(-0.5)*Float(angleMultiplyer.rawValue-1))
